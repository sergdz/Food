...
function slider({container, slide, nextArrow, prevArrow, totalCounter, currentCounter, wrapper, field}) {
    ...
    const regularWidth = (str) => +str.replace(/\D/g, '')
    ...
    next.addEventListener('click', () => {
        if (offset == regularWidth(width) * (slides.length - 1)) {
            offset = 0;
        } else {
            offset += regularWidth(width);
    ...
    prev.addEventListener('click', () => {
        if (offset == 0) {
            offset = regularWidth(width) * (slides.length - 1);
        } else {
            offset -= regularWidth(width);
        }
    ...
    dots.forEach(dot => dot.addEventListener('click', (e) => {
        ...
        slideIndex = slideTo;
        offset = regularWidth(width) * (slideTo - 1);
...
window.addEventListener('DOMContentLoaded', function () {
    ...
    (0,_modules_timer__WEBPACK_IMPORTED_MODULE_6__["default"])('.timer', '2023-05-30');
